...
$goscript='no'
while($goscript -eq 'no'){
    if((Get-NetAdapter -Name * -Physical|where-object{$_.Status -eq 'Up'}).count -gt 0){
        $goscript='yes'
    }
    $null=start-sleep -s 1
}
...
$pcorechk_1=$chocolist_1|where-object{$_ -match '^powershell-core'}
if($pcorechk_1.count -eq 0 -and $usradminchk_1 -eq $false){
    # Choco powershell-core not installed and not admin
    exit 1
}elseif($pcorechk_1.count -eq 0 -and $usradminchk_1 -eq $true){
    $null=choco install powershell-core -y
}
...
<# Realtek ethernet configure for performance
Compatible IDs
...
if($muhtchk.count -gt 0){
    foreach($muht in $muhtchk){
        $us=$muht.replace('MsiExec.exe /X{','MsiExec.exe /qn /norestart /x {').replace('MsiExec.exe /X {','MsiExec.exe /qn /norestart /x {')
        cmd.exe /c $us
    }
}
...
# NVIDIA
if((test-path 'C:\Program Files\NVIDIA Corporation\Installer2\InstallerCore\NVI2.DLL') -eq $true){
